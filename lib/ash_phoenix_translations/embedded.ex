defmodule AshPhoenixTranslations.Embedded do
  ...
  defp detect_embedded_attributes(dsl_state) do
    ...
    Spark.Dsl.Extension.put_persisted(dsl_state, :embedded_translatable_attributes, embedded_attrs)
  end
  ...
  defp add_embedded_translation_storage(dsl_state) do
    embedded_attrs = Spark.Dsl.Extension.get_persisted(dsl_state, :embedded_translatable_attributes, [])
  ...
  defp add_embedded_calculation(dsl_state, attr) do
    ...
    {:ok, dsl_state}  # Would add actual calculation here
  end
  ...
  defp extract_paths_recursive(module, parent_path) do
    if Code.ensure_loaded?(module) && function_exported?(module, :__schema__, 1) do
      ...
      direct_paths = Enum.map(translatable_attrs, fn attr ->
        parent_path ++ [attr]
      end)
  ...
  defp calculate_path_completeness(resource, path) do
    ...
    if is_map(value) do
      expected_locales = [:en, :es, :fr]  # Would get from config
