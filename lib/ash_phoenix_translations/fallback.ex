defmodule AshPhoenixTranslations.Fallback do
  ...
  def get_translation(translations, locale, opts \\ []) when is_map(translations) do
    ...
    result = Enum.find_value(fallback_chain, fn loc ->
      case Map.get(translations, loc) do
        nil -> nil
        "" -> nil  # Treat empty strings as missing
        value -> value
      end
    end)
  ...
  def build_fallback_chain(locale, fallback, available_locales) do
    ...
    chain = if fallback && fallback not in chain do
      chain ++ [fallback]
    else
      chain
    end
    ...
    chain = if @default_locale not in chain do
  ...
  def validate_required(translations, required_locales) when is_map(translations) do
    ...
    missing = Enum.filter(required, fn locale ->
  ...
  defp normalize_locale(locale) when is_binary(locale), do: String.to_atom(locale)
